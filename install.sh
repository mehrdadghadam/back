...
MAX_TUNNELS_FILE="${APP_DIR}/max-tunnels"
...
get_max_tunnels() {
  local max=""
  if [[ -f "${MAX_TUNNELS_FILE}" ]]; then
    max="$(cat "${MAX_TUNNELS_FILE}" 2>/dev/null || true)"
  fi
  if [[ ! "${max}" =~ ^[1-9][0-9]?$|^100$ ]]; then
    mkdir -p "${APP_DIR}"
    read -r -p "Maximum number of tunnels [10]: " max
    max="${max:-10}"
    if [[ ! "${max}" =~ ^[1-9][0-9]?$|^100$ ]]; then
      echo "Invalid number. Choose 1-100."
      return 1
    fi
    printf '%s\n' "${max}" > "${MAX_TUNNELS_FILE}"
  fi
  echo "${max}"
}
...
set_max_tunnels() {
  local max
  mkdir -p "${APP_DIR}"
  read -r -p "Maximum number of tunnels [10] (1-100): " max
  max="${max:-10}"
  if [[ ! "${max}" =~ ^[1-9][0-9]?$|^100$ ]]; then
    echo "Invalid number. Choose 1-100."
    return 1
  ...
  printf '%s\n' "${max}" > "${MAX_TUNNELS_FILE}"
  echo "[✓] Maximum tunnels set to ${max}."
}

select_tunnel() {
  local id max
  max="$(get_max_tunnels)" || return 1
  read -r -p "Tunnel number [1-${max}]: " id
  if [[ ! "${id}" =~ ^[0-9]+$ ]] || (( id < 1 || id > max )); then
    echo "Invalid tunnel number. Use 1-${max}."
    return 1
  fi
  echo "${id}"
}
...
uninstall_all() {
  echo
  read -r -p "Remove ALL tunnels, Backhaul binary and configs? [y/N]: " answer
  [[ "${answer}" =~ ^[Yy]$ ]] || return 0

  # Remove every configured tunnel service/config, not just 1 and 2.
  shopt -s nullglob
  local file id
  for file in /etc/systemd/system/backhaul-*.service; do
    id="${file##*/backhaul-}"
    id="${id%.service}"
    [[ "${id}" =~ ^[0-9]+$ ]] || continue
  ...
  for file in "${APP_DIR}"/server-*.toml "${APP_DIR}"/client-*.toml; do
    [[ -e "${file}" ]] || continue
    rm -f "${file}"
  done
  shopt -u nullglob
